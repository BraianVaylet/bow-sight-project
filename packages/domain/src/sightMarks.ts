...
import { INDOOR_DISTANCE_M, PRINT_STEP_M } from './constants';
...
export interface PrintableSightMark extends ComputedSightMark {
  /** true => la distancia coincide con una marca que cargó el arquero. */
  measured: boolean;
}

/**
 * Listado para llevar impreso al torneo: una fila cada `step` metros (2 por
 * defecto) desde `from` (la de sala, 18 m) hasta la distancia máxima registrada,
 * más esa máxima cuando no cae justo en la grilla. Marca cuáles son distancias
 * cargadas por el arquero (`measured`) y descarta las que quedan fuera de la
 * escala de la mira. Si la máxima registrada es menor que `from`, devuelve solo
 * la fila de `from`.
 */
export function printableSightMarks(
  model: SightModel,
  userDistances: number[],
  opts: { from?: number; step?: number; scaleMin: number; scaleMax: number },
): PrintableSightMark[] {
  const { from = INDOOR_DISTANCE_M, step = PRINT_STEP_M, scaleMin, scaleMax } = opts;
  if (step <= 0) throw new Error('printableSightMarks: el paso debe ser mayor que 0.');

  const targets: number[] = [];
  if (model.max <= from) {
    targets.push(from);
  } else {
    // Se recalcula desde `from` en cada paso (en vez de acumular) para no
    // arrastrar error de punto flotante con pasos decimales.
    for (let i = 0; ; i++) {
      const d = Math.round((from + i * step) * 10) / 10;
      if (d > model.max) break;
      targets.push(d);
    }
    if (targets[targets.length - 1] !== model.max) targets.push(model.max);
  }

  const userSet = new Set(userDistances);
  const out: PrintableSightMark[] = [];
  for (const d of targets) {
    const { mark, interpolated } = model.markAt(d);
    if (mark < scaleMin || mark > scaleMax) continue;
    out.push({
      distanceM: d,
      scaleValue: roundMark(mark),
      interpolated,
      measured: userSet.has(d),
    });
  }
  return out;
}
